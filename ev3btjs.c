...
int ev3_getbattery(void){
	char cmd[10]={0x08,0x00,0x03,0x00,0x00,0x01,0x00,0x81,0x12,0x60};//read batt level 
//	char cmd[10]={0x08,0x00,0x03,0x00,0x00,0x04,0x00,0x81,0x01,0x60};//read batt voltage 
	...
	int replylength;
	int error =0;
	if((result = write(nxtSocket, cmd, 10))<0){
		perror("error sending getbatterylevel command");
		return result;
	}
//	if((result=read(nxtSocket, reply,9)) <0){
	if((result=read(nxtSocket, reply,6)) <0){
		perror("error receiving getbattrylevel reply");
		return result;
	}

//	printf("%x %x %x %x %x %x %x %x %x\n\r",reply[0],reply[1],reply[2],reply[3],reply[4],reply[5],reply[6],reply[7],reply[8]);
//	printf("%x %x %x %x %x %x\n\r",reply[0],reply[1],reply[2],reply[3],reply[4],reply[5]);
...
//	blevel = reply[5] || (reply[6]<<8) || (reply[7]<<16) || (reply[8]<<24);
	blevel = reply[5];
...
int ev3_move_b(int b,int scale){
	char bmd[15]={0x0d,0x00,0x02,0x00,0x80,0x00,0x00,0xa5,0x00,0x02,0x81,0x00,0xa6,0x00,0x02};//move
	int result=0;
	b=b/scale;
	if(b>100) b=100;
	if(b<-100) b=-100;
	...
	btBusy=1;
//	printf("b %d\n\r",b);
	if((result = write(nxtSocket, bmd, 15))<0){
		perror("error sending move B motor command");
	//	return result;
		}
	btBusy= 0;
...
int ev3_move_c(int c,int scale){
	char cmd[15]={0x0d,0x00,0x02,0x00,0x80,0x00,0x00,0xa5,0x00,0x04,0x81,0x00,0xa6,0x00,0x04};//move
	int result=0;
	...
	btBusy=1;
//	printf("c %d\n\r",c);
	if((result = write(nxtSocket, cmd, 15))<0){
		perror("error sending move C motor command");
	//	return result;
		}
	btBusy= 0;
...
int ev3_move_sync(int c,int sync){
 	char cmd[15]={0x0d,0x00,0x02,0x00,0x80,0x00,0x00,0xa5,0x00,0x06,0x81,0x00,0xa6,0x00,0x06};//move
	int result=0;
...
int ev3_move_mot(unsigned char b, unsigned char c){
	char cmd[20]={0x11,0x00,0x02,0x00,0x80,0x00,0x00,0xa5,0x00,0x04,0x81,0x00,0xa5,0x00,0x02,0x81,0x00,0xa6,0x00,0x06};//move
	int result=0;
	...
	btBusy=1;
	printf("move sync %d\n\r",c);
	if((result = write(nxtSocket, cmd, 20))<0){
		perror("error sending move C motor command");
	//	return result;
		}
...
int ev3_move_arm(char a){
 	char cmd[15]={0x0d,0x00,0x02,0x00,0x80,0x00,0x00,0xa5,0x00,0x08,0x81,0x00,0xa6,0x00,0x08};//move
	int result;

	cmd[11]=a;
	btBusy=1;

//	printf("arm motor on %x\n\r",nxtSocket);
	if((result = write(nxtSocket, cmd, 15))<0){
		perror("error sending move command");
		printf("BT error\n\r");
	//	return result;
	}
	btBusy=0;
...
int ev3_move_test(int a){
	unsigned char amb[18]={0x10,0x00,0x02,0x00,0x80,0x00,0x00,0xa5,0x00,0x04,0x40,0xa5,0x00,0x02,0x40,0xa6,0x00,0x06};//move
	unsigned char amc[11]={0x09,0x00,0x03,0x00,0x80,0x00,0x00,0xa3,0x00,0x06,0x00};//stop
	unsigned char amd[17]={0x0f,0x00,0x01,0x00,0x80,0x00,0x00,0x94,0x01,0x81,0x02,0x82,0xe8,0x03,0x82,0xf4,0x01};//sound
	int result;
	btBusy=1;

	printf("ev3 sound %x\n\r",nxtSocket);
	if((result = write(nxtSocket, amd, 17))<0){
		perror("error sending sound command");
		printf("Openn BT conn\n\r");
	//	return result;
	}
	sleep(1);
	printf("motor on %x\n\r",nxtSocket);
	if((result = write(nxtSocket, amb, 18))<0){
		perror("error sending move command");
		printf("BT error\n\r");
	//	return result;
	}
	sleep(1);
	printf("motor off%x\n\r",nxtSocket);
	if((result = write(nxtSocket, amc, 11))<0){
...
int ev3_init(){
	unsigned char ame[20]={0x12,0x00,0x02,0x00,0x80,0x00,0x00,0xae,0x00,0x06,0x81,0x32,0x00,0x82,0x84,0x03,0x82,0xb4,0x00,0x01};//move
	int result;
	btBusy=1;

	printf("motor move %x\n\r",nxtSocket);
	if((result = write(nxtSocket, ame, 20))<0){
		perror("error sending move command");
		printf("BT error\n\r");
	//	return result;
	}
...
//	digitalWrite(PinA, 0);
	...
	blevel = ev3_getbattery();
	printf("battery level: %d%c\n",blevel,0x25);
	...
	while(waitForStart){
		...
		if(diff_time >= 1){
			...
			if(blevel<0){
				...
				sprintf (buf2, " Battery: %d%c  ",blevel,0x25);
				lcdPosition (fd1, 0, 0); lcdPuts (fd1, " GLXP  MoonBots ");
			...
			switch(js.type & ~JS_EVENT_INIT) {
				case JS_EVENT_BUTTON:
					...
					if(button[9]){
	 				...
	 				if(button[0] && !button[2]) apwr = 20;
					else if(!button[0] && button[2]) apwr = -20;
					else apwr = 0;
...
//					  nxt_move_arm(apwr);
					...
					if(mvpwr!=preMvpwr){
					  ev3_move_sync(botDir*mvpwr,motorNotSync);
